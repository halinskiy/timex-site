...
function Section({ id, bg = "bg", children }: { id?: string; bg?: "bg" | "surface"; children: ReactNode }) {
  return (
    <section id={id} style={{ background: bg === "surface" ? "var(--surface)" : "var(--bg)" }} className="py-[clamp(88px,10vw,150px)]">
...
const shadow = "drop-shadow(0 60px 110px rgba(0,0,0,0.6))";
...
function Window({ kind, width = 400, height = 680, className = "" }: { kind: "timeline" | "export" | "stats" | "project"; width?: number; height?: number; className?: string }) {
  return (
    <div className={className} style={{ width, height, maxWidth: "100%", filter: shadow }}>
      <Screen kind={kind} className="h-full w-full" />
    </div>
  );
}

// The hero centrepiece: a real, live Timex that types itself and takes over the
// moment you click in. This is the product, not a picture of it.
function HeroTerminal() {
  const tx = useTimex({ autoplay: !new URLSearchParams(window.location.search).has("noauto") });
  return (
    <div className="mx-auto lg:ml-auto lg:mr-0" style={{ width: 440, maxWidth: "100%", height: 720, filter: shadow }}>
...
export default function App() {
  return (
    <div id="top" style={{ background: "var(--bg)" }}>
      ...
      <Section bg="bg">
        <div className="u-grid items-center gap-y-16 pt-24">
          <div className="col-span-12 lg:col-span-6">
            <Reveal>
              <h1 className="display font-semibold" style={{ fontSize: "clamp(46px,6.4vw,94px)", lineHeight: 0.96, letterSpacing: "-0.035em" }}>
                Type. The hour<br />logs itself.
                <span
                  className="ml-3 inline-block align-[-0.08em]"
                  style={{ width: "0.46em", height: "0.86em", background: "var(--amber)", animation: "caret-blink 1s steps(1) infinite" }}
                />
              </h1>
            </Reveal>
            <Reveal delay={0.08}>
              <p className="mt-8 text-[clamp(18px,1.5vw,22px)] leading-[1.55]" style={{ color: "var(--muted)", maxWidth: "38ch" }}>
                A keyboard-first time tracker for macOS, in a real terminal window. Name what you're working on and it's logged. When the week is done, hand your client a single link.
              </p>
            </Reveal>
            <Reveal delay={0.16}>
        ...
        <div className="u-grid items-center gap-y-16">
          <div className="col-span-12 lg:col-span-5">
            <SectionHead
              ...
              subhead="No browser, no Electron. Type a line and the clock runs against that project. Type another and it switches. Everything else is one slash-command away, so your hands never leave the keys."
            />
            <Reveal delay={0.1}>
              <ul className="mt-12">
      ...
      {/* ── Commands ──────────────────────────────────────────── */}
      <Section id="commands" bg="surface">
        <div className="u-grid">
          <div className="col-span-12 lg:col-span-9">
          ...
          {[
            ...
            <Reveal key={cmd.c} delay={(i % 3) * 0.05} className="col-span-12 md:col-span-6 lg:col-span-4">
              <div className="border-t pt-8" style={{ borderColor: "var(--border)" }}>
                <span className="font-mono text-[20px]" style={{ color: "var(--ink)" }}>
                  <span style={{ color: "var(--amber)" }}>/</span>{cmd.c.slice(1)}
                </span>
      ...
      {/* ── CTA ───────────────────────────────────────────────── */}
      <Section id="get" bg="bg">
        <div className="u-grid">
          <div className="col-span-12 lg:col-span-10">
            <Reveal>
              <h2 className="display font-semibold" style={{ fontSize: "clamp(42px,5.8vw,96px)", lineHeight: 0.97, letterSpacing: "-0.03em" }}>
